...
def deploy_to_azure():
    ...
    if run_command("azd --version", "Checking Azure Developer CLI",
                   shell=True):
    ...
    if run_command("func --version", "Checking Azure Functions Core Tools"):
        ...
        if run_command("func azure functionapp publish tca-irr-functions",
...
def deploy_to_app_service():
    ...
    # Deploy code
    if run_command(
            f"az webapp up --name {app_name} --resource-group {resource_group} --runtime 'PYTHON|3.12'",
