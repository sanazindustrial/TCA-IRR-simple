...
def parse_json_list_field(value: Any) -> list:
    """Safely parse JSON list field that may be stored as string in database."""
    if value is None:
        return []
...
async def get_all_settings_versions(include_archived: bool = Query(
    False, description="Include archived versions")):
    ...
    try:
        async with db_manager.get_connection() as conn:
            ...
            versions = []
            for row in rows:
                version = SettingsVersion(id=row['id'],
        ...
        async with db_manager.get_connection() as conn:
            ...
            if not row:
                raise HTTPException(status_code=404,
                                    detail="No active settings version found")
            ...
            if not version_row:
                raise HTTPException(status_code=404,
                                    detail="Settings version not found")
            ...
            return SettingsVersion(
                ...
                tca_categories=[
                    TCACategory(id=r['id'],
                                ...
                                factors=parse_json_list_field(r['factors']))
                    for r in tca_rows
                ])
            ...
            if data.copy_from_version:
                ...
                for ms in DEFAULT_MODULE_SETTINGS:
                    await conn.execute(
                        ...
                        VALUES ($1, $2, $3, $4, $5, $6)
                    """, version_id, ms.module_id.value, ms.module_name,
                        ms.weight, ms.is_enabled, ms.priority)
                ...
                for tc in DEFAULT_TCA_CATEGORIES:
                    ...
                    """, version_id, tc.category_name, tc.category_order,
                        tc.weight, tc.medtech_weight, tc.is_active,
...
async def update_settings_version(version_id: int,
                                  data: SettingsVersionUpdate):
...
@router.put("/versions/{version_id}/modules/{module_id}",
            response_model=ModuleSetting)
async def update_module_setting(version_id: int, module_id: str,
    ...
    try:
        async with db_manager.get_transaction() as conn:
            ...
            return ModuleSetting(id=row['id'],
...
@router.get("/versions/{version_id}/tca-categories",
            response_model=List[TCACategory])
...
@router.put("/versions/{version_id}/tca-categories/{category_id}",
            response_model=TCACategory)
async def update_tca_category(version_id: int, category_id: int,
    ...
    try:
        async with db_manager.get_transaction() as conn:
            ...
            if not row:
                raise HTTPException(status_code=404,
                                    detail="TCA category not found")

            return TCACategory(id=row['id'],
                               ...
                               or None,
                               description=row['description'],
                               factors=parse_json_list_field(row['factors']))
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_simulation_runs(user_id: Optional[int] = None,
    ...
    try:
        async with db_manager.get_connection() as conn:
            ...
            return [
                SimulationRun(id=r['id'],
                              settings_version_id=r['settings_version_id'],
                              user_id=r['user_id'],
                              company_name=r['company_name'],
                              analysis_id=r['analysis_id'],
                              tca_score=float(r['tca_score'])
                              if r['tca_score'] else None,
    ...
    try:
        async with db_manager.get_transaction() as conn:
            ...
            if tca_categories and data.adjusted_scores and 'tca' in data.adjusted_scores:
                ...
                total_weight = sum(c.weight for c in tca_categories
                                   if c.is_active)
                if total_weight > 0:
                    weighted_sum = 0
                    for cat in tca_categories:
                        if cat.is_active:
                            ...
                            cat_score = next(
                                (s.get('score', 5.0) for s in tca_data
                                 if s.get('category') == cat.category_name),
                                5.0)
                ...
                if ms.is_enabled and ms.module_id.value != 'tca':
                    if data.adjusted_scores and ms.module_id.value in data.adjusted_scores:
                        ...
                        if isinstance(scores, list) and len(scores) > 0:
                            module_scores[ms.module_id.value] = sum(
                                s.get('score', 5.0)
    ...
    try:
        async with db_manager.get_connection() as conn:
            ...
            return SimulationRun(
                ...
                tca_score=float(row['tca_score'])
                if row['tca_score'] else None,
...
async def compare_simulation_versions(version_id_1: int, version_id_2: int):
    ...
    try:
        ...
        for ms1 in v1.module_settings:
            ms2 = next(
                (m
                 for m in v2.module_settings if m.module_id == ms1.module_id),
                None)
            if ms2:
                if ms1.weight != ms2.weight or ms1.is_enabled != ms2.is_enabled:
                    module_diff.append({
                        ...
                        "version_1": {
                            "weight": ms1.weight,
                            "is_enabled": ms1.is_enabled
                        },
                        "version_2": {
                            "weight": ms2.weight,
                            "is_enabled": ms2.is_enabled
                        }
                    })

        tca_diff = []
        for tc1 in v1.tca_categories:
            tc2 = next((t for t in v2.tca_categories
                        if t.category_name == tc1.category_name), None)
            if tc2:
                if tc1.weight != tc2.weight or tc1.is_active != tc2.is_active:
                    tca_diff.append({
                        ...
                        "version_1": {
                            "weight": tc1.weight,
                            "is_active": tc1.is_active
                        },
                        "version_2": {
                            "weight": tc2.weight,
                            "is_active": tc2.is_active
                        }
                    })
