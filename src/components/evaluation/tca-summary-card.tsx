...
export function TcaSummaryCard({ initialData }: TcaSummaryCardProps) {
  ...
  const normalizedData = React.useMemo(() => {
    ...
    // If score > 10, it's likely the sum of weighted * 10 instead of correctly weighted
    if (compositeScore > 10) {
      // Try to recalculate from categories if available
      if (initialData.categories && initialData.categories.length > 0) {
        ...
        // Fallback: divide by 10 to normalize to 0-10 scale
        compositeScore = compositeScore / 10;
      }
    }

    // Ensure score is clamped to 0-10 range
