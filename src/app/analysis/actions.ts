...
export async function runAnalysis(
  ...
  try {
    ...
    if (!['general', 'medtech'].includes(framework)) {
      ...
      return buildFallbackData(userData);
    }
    ...
    // CRITICAL FIX: If no TCA categories from backend, return fallback — must not save sample data to DB
    if (tcaCategories.length === 0) {
      console.warn('No TCA categories from backend, returning fallback data');
      return buildFallbackData(userData);
    }
