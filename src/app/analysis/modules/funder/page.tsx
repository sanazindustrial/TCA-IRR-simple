...
export default function FunderFitConfigPage() {
  ...
  const handleWeightChange = (sector: 'tech' | 'med_life', dimension: string, value: string) => {
    const newWeights = { ...weights };
    (newWeights[sector] as any)[dimension] = parseFloat(value) || 0;
  ...
  const weightDimensions = Object.keys(weights.tech);
  ...
  const handleNormalize = (sector: 'tech' | 'med_life') => {
    ...
    for (const key in currentWeights) {
        normalizedSectorWeights[key] = (currentWeights as any)[key] / total;
    }
                        ...
                        <TableBody>
                            {weightDimensions.map(dim => {
                                ...
                                <TableRow key={dim}>
                                    ...
                                    <TableCell>
                                        <Input type="number" value={(weights.tech as any)[dim]} onChange={e => handleWeightChange('tech', dim, e.target.value)} className="h-8 w-24 text-center mx-auto" step="0.01"/>
                                    </TableCell>
                                     <TableCell>
                                        <Input type="number" value={(weights.med_life as any)[dim]} onChange={e => handleWeightChange('med_life', dim, e.target.value)} className="h-8 w-24 text-center mx-auto" step="0.01"/>
                                    </TableCell>
                                </TableRow>
                            )})}
                        </TableBody>
