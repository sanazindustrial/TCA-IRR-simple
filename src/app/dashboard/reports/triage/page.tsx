...
export default function WhatIfAnalysisPage() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      // Use type assertion for strategicFitData as schema is loosely typed
      const sfData = data.strategicFitData as { data?: Array<{ pathway: string; signal: string }> } | null;
      if (sfData?.data) {
            initialScores['strategicFit'] = sfData.data.map(s => ({ id: s.pathway, category: s.pathway, score: s.signal === 'green' ? 8 : s.signal === 'yellow' ? 6 : 4 }));
      }
