...
import { Timer, Eye, ArrowLeft } from 'lucide-react';
...
function ReportView({ analysisData, isPreview = false, visibleSections }: { analysisData: ComprehensiveAnalysisOutput, isPreview?: boolean, visibleSections: ReportSection[] }) {
    ...
    const reportComponents: Record<string, React.ReactNode> = {
        'dd-executive-summary': <ExecutiveSummary />,
        'dd-tca-scorecard': analysisData.tcaData && <TcaScorecard initialData={analysisData.tcaData} />,
        'dd-weighted-score-breakdown': <WeightedScoreBreakdown />,
        'dd-risk-flags': analysisData.riskData && <RiskFlags initialData={analysisData.riskData} />,
        'dd-macro-trend': analysisData.macroData && <MacroTrendAlignment data={analysisData.macroData} />,
        'dd-benchmark': analysisData.benchmarkData && <BenchmarkComparison initialData={analysisData.benchmarkData} />,
        'dd-growth-classifier': analysisData.growthData && Object.keys(analysisData.growthData).length > 0 && <GrowthClassifier />,
        'dd-strategic-fit': analysisData.strategicFitData && Object.keys(analysisData.strategicFitData).length > 0 && <StrategicFitMatrix />,
        'dd-consistency-check': <ConsistencyCheck />,
        'dd-gap-analysis': analysisData.gapData && Object.keys(analysisData.gapData).length > 0 && <GapAnalysis />,
        'dd-founder-fit': analysisData.founderFitData && Object.keys(analysisData.founderFitData).length > 0 && <FunderFitAnalysis />,
        'dd-team-assessment': analysisData.teamData && Object.keys(analysisData.teamData).length > 0 && <TeamAssessment />,
        ...
        'dd-export-links': (
            <Card>
                <CardHeader>
                <CardTitle>Admin Final Approval Panel</CardTitle>
                </CardHeader>
                <CardContent className="flex flex-col md:flex-row gap-4">
                <div className="flex-1 space-y-4">
                    <Button size="lg" className="w-full">Lock & Approve for Investment</Button>
                    <Button size="lg" variant="secondary" className="w-full">Hold (Further Review Needed)</Button>
                    <Button size="lg" variant="destructive" className="w-full">Reject & Archive</Button>
                </div>
        ...
        <div className="space-y-6">
            {isPreview && (
                 <Alert variant="default" className="bg-primary/10 border-primary/30 text-primary-foreground">
                    ...
                    <AlertTitle className="text-primary">Training Mode: Sample Report</AlertTitle>
                    <AlertDescription className="flex justify-between items-center text-primary/90">
                        This is a sample Due Diligence report to demonstrate the layout and components for training purposes. The data is not real.
                    </AlertDescription>
                </Alert>
            )}
            {sectionIsVisible('quick-summary') && <QuickSummary />}
    ...
    >
        ...
        <div className="container mx-auto p-4 md:p-8">
            <header className="mb-12">
                 <Link href="/dashboard/help/understanding-your-report" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-primary mb-4">
                ...
                <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto text-center">
                    This is a sample Due Diligence report for training and demonstration purposes. The data is not real.
                </p>
            </header>
            
            <ReportView analysisData={sampleAnalysisData} isPreview={true} visibleSections={visibleSections} />
        </div>
        </main>
    </EvaluationProvider>
  );
