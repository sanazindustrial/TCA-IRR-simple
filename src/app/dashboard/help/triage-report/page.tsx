...
function ReportView({ analysisData, visibleSections }: { analysisData: ComprehensiveAnalysisOutput | null, visibleSections: ReportSection[] }) {
    ...
    const reportComponents: Record<string, React.ReactNode> = {
        ...
        'growth-classifier': analysisData.growthData && Object.keys(analysisData.growthData).length > 0 && <GrowthClassifier />,
        ...
        'admin-approval-panel': (
            <Card>
                ...
                <CardContent className='flex gap-4'>
                    <Button variant="outline">Generate Triage Report</Button>
                    <Button>Proceed to Full Screening</Button>
        ...
        <div className="space-y-6">
            <Alert variant="default" className="bg-primary/10 border-primary/30 text-primary-foreground">
                ...
                <AlertDescription className="flex justify-between items-center text-primary/90">
                    This is a sample Triage report to demonstrate the layout and components for training purposes. The data is not real.
                </AlertDescription>
            </Alert>
...
const defaultStandardSections: ReportSection[] = [
...
export default function TriageReportPage() {
  ...
  useEffect(() => {
      ...
      const configKey = isPrivilegedUser ? 'report-config-triage-admin' : 'report-config-triage-standard';
      try {
          ...
          if (savedConfig) {
              ...
              const defaultConfig = isPrivilegedUser ? defaultStandardSections.map(s => ({...s, description: ''})) : defaultStandardSections.map(s => ({...s, description: ''})); // A more complete default can be defined
              setVisibleSections(defaultConfig as ReportSection[]);
          }
      } catch (e) {
          console.error("Failed to load triage report configuration", e);
          const defaultConfig = isPrivilegedUser ? defaultStandardSections.map(s => ({...s, description: ''})) : defaultStandardSections.map(s => ({...s, description: ''}));
          setVisibleSections(defaultConfig as ReportSection[]);
      }
  }, [isPrivilegedUser]);
        ...
        <div className="container mx-auto p-4 md:p-8">
            <header className="mb-12">
                ...
                <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto text-center">
                    This is a sample Triage report for training and demonstration purposes. Use the switch above to see how the report changes based on user role.
                </p>
            </header>
            
            <ReportView analysisData={sampleAnalysisData} visibleSections={visibleSections} />
        </div>
        </main>
    </EvaluationProvider>
  );
