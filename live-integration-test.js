...
async function runTest(testName, testFunction) {
    ...
    try {
        ...
        if (result && result.success) {
            ...
            testResults.failed++;
            testResults.details.push({ name: testName, status: 'FAIL', message: result ? result.message : 'Test failed' });
...
async function testFileUpload() {
    try {
        const mockFiles = [
            { name: 'business_plan.pdf', size: 1024000, type: 'application/pdf' },
            { name: 'financials.xlsx', size: 512000, type: 'application/vnd.ms-excel' }
        ];
        ...
        if (!response.ok) {
            return { success: false, message: `File upload failed with status ${response.status}` };
        }
        ...
        if (data.status === 'success' && data.files_processed === mockFiles.length) {
            ...
            return { success: false, message: 'File upload response invalid' };
        }
    } catch (error) {
        return { success: false, message: `File upload error: ${error.message}` };
    }
}
...
async function testUrlFetch() {
    try {
        ...
        if (!response.ok) {
            return { success: false, message: `URL fetch failed with status ${response.status}` };
        }
        ...
        if (data.status === 'success' && data.urls_processed === mockUrls.length) {
            ...
            return { success: false, message: 'URL fetch response invalid' };
        }
    } catch (error) {
        return { success: false, message: `URL fetch error: ${error.message}` };
    }
}
...
async function testTCAAnalysis() {
    try {
        ...
        if (!response.ok) {
            ...
            return { success: false, message: `Analysis failed: ${response.status} - ${errorText}` };
        }
        ...
        if (!data.final_tca_score || !data.scorecard || !data.risk_assessment) {
            return { success: false, message: 'Incomplete analysis result structure' };
        }
        
        if (data.final_tca_score < 0 || data.final_tca_score > 100) {
            return { success: false, message: `Invalid TCA score: ${data.final_tca_score}` };
        }
        ...
        return { success: false, message: `TCA analysis error: ${error.message}` };
    }
}
...
async function testFrontendHealth() {
    try {
        ...
        if (response.status === 200 || response.status === 404) {
            ...
            return { success: true, message: 'Frontend server is running and accessible' };
        } else {
            return { success: false, message: `Frontend returned status ${response.status}` };
        }
    } catch (error) {
        return { success: false, message: `Frontend connection failed: ${error.message}` };
    }
}
...
async function runLiveTests() {
    ...
    const report = {
        ...
        recommendations: testResults.passed === testResults.total ? 
            ['Ready for manual testing', 'All endpoints functional', 'Complete pipeline operational'] :
            ['Review failed tests', 'Check server connectivity', 'Verify API responses']
    };
