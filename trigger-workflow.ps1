...
$token = [CM5]::Get('git:https://github.com')
...
try {
  $resp = Invoke-RestMethod -Uri "https://api.github.com/repos/sanazindustrial/TCA-IRR-simple/actions/workflows/main_tca-irr.yml/dispatches" -Method POST -Headers $headers -Body $body -ContentType "application/json"
  Write-Host "[SUCCESS] Workflow triggered!" -ForegroundColor Green
} catch {
  $statusCode = $_.Exception.Response.StatusCode.value__
  Write-Warning "Status: $statusCode"
  Write-Warning $_.Exception.Message
}

# Check latest workflow runs
Write-Host "`nLatest workflow runs:"
...
foreach ($r in $runs.workflow_runs) {
  Write-Host "  #$($r.run_number) $($r.status) - $($r.conclusion) - $($r.created_at) - $($r.html_url)"
}
